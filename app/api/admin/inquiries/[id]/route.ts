...
export async function GET(req:Request,ctx:Context){try{await staff();const {id}=await ctx.params;if(!validId(id))throw new HttpError(404,'Not found');
 const [rows,events,mail]=await Promise.all([sb('/rest/v1/inquiries?id=eq.'+id+'&select=*'),sb('/rest/v1/inquiry_events?inquiry_id=eq.'+id+'&order=created_at.desc&limit=200&select=actor,note,created_at'),sb('/rest/v1/inquiry_outbox?inquiry_id=eq.'+id+'&select=kind,state,attempts,provider_id')]);
 if(!rows.length)throw new HttpError(404,'Not found');const value={inquiry:rows[0],events,mail};
