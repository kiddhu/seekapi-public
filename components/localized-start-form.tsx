...
export function LocalizedStartForm({copy,russian=false,locale='en'}:{copy:FormCopy;russian?:boolean;locale?:Locale}){return process.env.NEXT_PUBLIC_INQUIRIES_ENABLED==='1'?<InquiryForm locale={russian?'ru':locale} services={copy.options}/>:<LegacyLocalizedStartForm copy={copy} russian={russian}/>;}
function LegacyLocalizedStartForm({copy, russian=false}:{copy:FormCopy;russian?:boolean}){
