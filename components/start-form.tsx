    ...
    <form aria-label={audience === 'agent' ? 'Agent handoff draft' : 'Company issue draft'} aria-describedby="sensitive-note" onSubmit={validate} onReset={() => setMessage('Draft cleared. Nothing was sent.')} noValidate>
      ...
      <div className="form-grid">
        ...
        {audience === 'company' ? <>
          ...
          <div className="field"><label htmlFor="task_type">Task class</label><select id="task_type" name="task_type" required defaultValue=""><option value="">Select AH01–AH08</option>{['AH01 Supplier contact','AH02 Discovery & qualification','AH03 RFQ execution','AH04 Sample / NPI follow-up','AH05 Physical verification coordination','AH06 Quality / exception recovery','AH07 Delivery / local coordination','AH08 Research + human verification'].map(value => <option key={value}>{value}</option>)}</select></div>
      ...
      <div className="button-row form-actions"><button className="button" type="submit">Validate preview draft</button><button className="button button-ghost" type="reset">Clear draft</button></div>
